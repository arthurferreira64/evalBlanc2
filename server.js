...
app.get('/api/vehicles', (req, _res, next) => {
    ...
    const sql = `
        SELECT vehicle.*, users.lastname, users.firstname 
        FROM vehicle 
        JOIN users ON vehicle.client_id = users.id
    `;
...
app.post('/api/vehicles', (req, _res, next) => {
    ...
    const {brand, model, year, client_id, registration_plate} = req.body;
    const sql = 'INSERT INTO vehicle (brand, model, year, client_id, registration_plate) VALUES (?, ?, ?, ?, ?)';
    db.query(sql, [brand, model, year, client_id, registration_plate], (err, result) => {
        if (err) {
            res.status(500).send('Server error');
            return;
        }
...
app.put('/api/vehicles/:id', (req, _res, next) => {
    ...
    const {id} = req.params;
    const {brand, model, year, client_id, registration_plate} = req.body;
    const sql = 'UPDATE vehicle SET brand = ?, model = ?, year = ?, client_id = ?, registration_plate = ? WHERE id = ?';
    db.query(sql, [brand, model, year, client_id, registration_plate, id], (err, result) => {
        if (err) {
            res.status(500).send('Server error');
            return;
        }
...
app.get('/api/clients', (req, _res, next) => {
    ...
    const sql = 'SELECT id, lastname, firstname FROM users WHERE role = ?';
    db.query(sql, ['client'], (err, results) => {
        if (err) {
            console.error(err);
            res.status(500).send('Server error');
